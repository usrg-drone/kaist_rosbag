...
## Important Notes
    Directory Conflict: The output directory must not exist prior to recording. The system automatically generates a unique timestamp-based folder.
    Script Deployment: The script executed at runtime is located in the install/ directory. 
    After modifying config/kros2.sh, you must rebuild the workspace (colcon build) or manually copy the script to the install path.
    Data Safety: Periodic disk synchronization is performed to reduce data loss in case of unexpected power failure.
